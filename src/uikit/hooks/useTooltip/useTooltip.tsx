...
const useTooltip = (content: React.ReactNode, options: TooltipOptions): TooltipRefs => {
  ...
  const { styles, attributes, update } = usePopper(targetElement, tooltipElement, {
    placement,
    modifiers: [
      {
        name: 'arrow',
  ...
  const TooltipContainer = CustomTooltipContainer || StyledTooltip
  const tooltip = (
    <TooltipContainer ref={setTooltipElement} style={styles.popper} {...attributes.popper}>
      {content}
      {showArrow && <Arrow ref={setArrowElement} style={styles.arrow} />}
    </TooltipContainer>
  )

  const portal = disablePortal ? null : getPortalRoot()
